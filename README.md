...
curl -X POST "$PUBLIC_URL/v1/chat/completions" -H "Content-Type: application/json" -d '{
  "model": "gpt-oss:20b",
  "messages": [
    {"role": "user", "content": "Explain why open AI infrastructure matters."}
  ]
}'
```
...
You now have: - OpenAI-compatible inference - Observability endpoint -
Public API - Free GPU-backed model

All running on distributed compute.
